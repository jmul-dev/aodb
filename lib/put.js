 ...
 * aodb's put() requires writerSignature and writerAddress (publicKey) in addition to hyperdb's existing put params
 */
function put (db, clock, heads, key, value, writerSignature, writerAddress, opts, cb) {
	if (typeof opts === 'function') return put(db, clock, heads, key, value, writerSignature, writerAddress, null, opts)
	var req = new PutRequest(db, key, value, writerSignature, writerAddress, clock, opts)
	req.start(heads, cb)
}

function PutRequest (db, key, value, writerSignature, writerAddress, clock, opts) {
	this.key = key
	this.pointerKey = (opts && opts.pointerKey) || ''
	this.schemaKey = (opts && opts.schemaKey) || ''
	this.value = value
	this.delete = !!(opts && opts.delete)
	this.ifNotExists = !!(opts && opts.ifNotExists)
	this.pointer = !!(opts && opts.pointer)
	this.noUpdate = !!(opts && opts.noUpdate)
	this.isSchema = !!(opts && opts.isSchema)
	this.isWildStringSchema = !!(opts && opts.isWildStringSchema)
	this.writerSignature = writerSignature
	this.writerAddress = writerAddress
	this.proofSignature = (opts && opts.proofSignature) || ''
	this.proofPayload = (opts && opts.proofPayload) || null
	this.rootHash = (opts && opts.rootHash) || ''
...
PutRequest.prototype._end = function (worker, err) {
	...
	if (!--this._active) {
		// If ifNotExists option == true or
		// existing entry's noUpdate == true or
		// existing entry isSchema == true, or
		// existing entry isWildStringSchema == true, means no rewrite
		if (worker.head.key === this.key && (this.ifNotExists || worker.head.noUpdate || this.noUpdate || worker.head.isSchema || this.isSchema || this.isWildStringSchema || worker.head.isWildStringSchema)) {

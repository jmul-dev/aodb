 ...
 * aodb's put() requires signature and publicKey in addition to hyperdb's existing put params
 */
function put (db, clock, heads, key, value, signature, publicKey, opts, cb) {
	if (typeof opts === 'function') return put(db, clock, heads, key, value, signature, publicKey, null, opts)
	var req = new PutRequest(db, key, value, signature, publicKey, clock, opts)
	req.start(heads, cb)
}

function PutRequest (db, key, value, signature, publicKey, clock, opts) {
	this.key = key
	this.value = value
	this.signature = signature
	this.writerAddress = publicKey
